  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 p-4 md:p-6 max-w-6xl mx-auto">
      <div className="flex-1 space-y-6">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight" data-testid="text-page-title">New Booking</h1>
          <p className="text-sm text-muted-foreground mt-1">Create a new cleaning job with locked pricing.</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><CalendarDays className="h-4 w-4" /> Customer & Schedule</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="date" type="date" value={scheduledDate} onChange={(e) => setScheduledDate(e.target.value)} data-testid="input-date" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="window">Arrival Window</Label>
                <Select value={arrivalWindow} onValueChange={setArrivalWindow}>
                  <SelectTrigger data-testid="select-arrival-window">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><Ruler className="h-4 w-4" /> Property Details</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label className="flex items-center gap-1.5"><Bed className="h-3.5 w-3.5" /> Bedrooms</Label>
                <div className="flex items-center gap-2">
                  <Button size="icon" variant="outline" onClick={() => setBedrooms(Math.max(0, bedrooms - 1))} data-testid="button-bed-minus">-</Button>
                  <span className="w-8 text-center font-medium" data-testid="text-bedrooms">{bedrooms}</span>
                  <Button size="icon" variant="outline" onClick={() => setBedrooms(bedrooms + 1)} data-testid="button-bed-plus">+</Button>
                </div>
              </div>
              <div className="space-y-2">
                <Label className="flex items-center gap-1.5"><Bath className="h-3.5 w-3.5" /> Bathrooms</Label>
                <div className="flex items-center gap-2">
                  <Button size="icon" variant="outline" onClick={() => setBathrooms(Math.max(0, bathrooms - 1))} data-testid="button-bath-minus">-</Button>
                  <span className="w-8 text-center font-medium" data-testid="text-bathrooms">{bathrooms}</span>
                  <Button size="icon" variant="outline" onClick={() => setBathrooms(bathrooms + 1)} data-testid="button-bath-plus">+</Button>
                </div>
              </div>
              <div className="space-y-2">
                <Label className="flex items-center gap-1.5"><Ruler className="h-3.5 w-3.5" /> Square Footage</Label>
                <Input type="number" value={sqft} onChange={(e) => setSqft(Number(e.target.value))} min={0} data-testid="input-sqft" />
              </div>
            </div>

            <div className="space-y-2">
              <Label className="flex items-center gap-1.5"><Percent className="h-3.5 w-3.5" /> Cleaning Frequency</Label>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><Tag className="h-4 w-4" /> Add-On Services</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="w-full lg:w-80 lg:sticky lg:top-6 lg:self-start space-y-4">
        <Card className="border-primary/20">
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2"><DollarSign className="h-4 w-4" /> Booking Summary</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="space-y-2 text-sm">
              <div className="flex justify-between" data-testid="text-summary-bedrooms">
                <span className="text-muted-foreground">{bedrooms} Bedrooms</span>
                <span>${bedrooms * PRICING_CONFIG.pricePerBedroom}</span>
              </div>
              <div className="flex justify-between" data-testid="text-summary-bathrooms">
                <span className="text-muted-foreground">{bathrooms} Bathrooms</span>
                <span>${bathrooms * PRICING_CONFIG.pricePerBathroom}</span>
              </div>
              <div className="flex justify-between" data-testid="text-summary-sqft">
                <span className="text-muted-foreground">{sqft} sq ft</span>
                <span>${sqftRange?.price || 210}</span>
              </div>
            ...
            {discount > 0 && (
              <div className="flex justify-between text-sm text-green-600 dark:text-green-400" data-testid="text-discount">
