  ...
  return (
    <div className="p-4 md:p-6 max-w-lg mx-auto space-y-6">
      <div className="text-center">
        <h1 className="text-2xl font-semibold tracking-tight text-white drop-shadow-lg" data-testid="text-page-title">Tech Clock-In</h1>
      ...
      {!isLoading && activeLog && (
        <GlassCard className="border-blue-400/30">
          <CardHeader className="pb-3 text-center">
            <CardTitle className="text-base flex items-center justify-center gap-2 text-white">
              <Timer className="h-4 w-4 text-blue-400" /> Active Session
            </CardTitle>
          </CardHeader>
          <CardContent className="text-center space-y-4">
            <div className="text-5xl font-mono font-bold tabular-nums tracking-wider text-white" data-testid="text-timer">
      ...
      {!isLoading && !activeLog && (
        <GlassCard>
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2 text-white"><Clock className="h-4 w-4" /> Today's Jobs</CardTitle>
          </CardHeader>
          <CardContent>
            {scheduledJobs.length === 0 ? (
              <div className="text-center py-8">
                <Clock className="h-10 w-10 mx-auto text-white/50 mb-3" />
                <p className="text-white/70">No scheduled jobs for today.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {scheduledJobs.map((job) => {
                  ...
                  return (
                    <div key={job.id} className="p-3 border border-white/20 rounded-md space-y-3" data-testid={`card-tech-job-${job.id}`}>
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        <div>
                          <p className="text-sm font-medium text-white">{cust?.name || "Customer"}</p>
                          <p className="text-xs text-white/70">{cust?.address || ""}</p>
                        </div>
      ...
      <GlassCard>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2 text-white"><Clock className="h-4 w-4" /> Recent Time Logs</CardTitle>
        </CardHeader>
        <CardContent>
          {timeLogs.length === 0 ? (
            <p className="text-sm text-white/70 text-center py-4">No time logs yet.</p>
          ) : (
            <div className="space-y-2">
              {timeLogs.slice(0, 10).map((log) => (
                <div key={log.id} className="flex items-center justify-between p-2 rounded-md border border-white/20 text-sm" data-testid={`row-timelog-${log.id}`}>
